                ...
                <tr>
                    ...
                    <td data-label="Actions">
                        <div class="osf-actions">
                            <a href="/admin/forms/<?= h((string) $id) ?>/edit" role="button" class="secondary"><?= icon('pencil') ?> Edit</a>
                            <form class="osf-inline-form"
                                  method="post"
                                  action="/admin/forms/<?= h((string) $id) ?>/<?= $active ? 'disable' : 'enable' ?>">
                                <input type="hidden" name="_csrf" value="<?= h($csrf) ?>">
                                <button type="submit" class="<?= $active ? 'osf-danger' : 'secondary' ?>"><?= $active ? 'Disable' : 'Enable' ?></button>
                            </form>
                        </div>
                    </td>
                </tr>
